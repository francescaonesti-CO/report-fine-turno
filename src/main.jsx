...
const anomalieText = official.anomalie || '-';
  ...
  const page2 = `<section class="page">${headerHtml('REPORT UFFICIALE DI TURNO - DETTAGLIO', subtitle)}${panel('Violazioni riscontrate','table',violTable)}<div class="detail-grid-2">${panel('Atti redatti','clip',attiBody)}<div>${panel('Esito turno','check',`<p>${esc(official.esiti || '-')}</p>`)}${panel('Comunicazioni E.Q.','doc',`<p>${esc(official.comunicazioneEq || '-')}</p>`)}</div></div><div class="detail-grid-2">${panel('Nota per il comandante','clipboard',`<p>${esc(official.notaComandante || '-')}</p>`)}${panel('Responsabile di turno','user',`<p class="center"><strong>${esc(official.qualifica || '-')}</strong></p><p class="center"><strong>${esc(official.ufficiale || '-')}</strong></p><div class="signature small-signature">Firma</div>`)}</div>${footerHtml(2)}</section>`;
