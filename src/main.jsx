...
<section className="loginScreen">
  <section className="loginCard">
    <div className="loginBrand">
      <img src="/POLIZIA.png" alt="Polizia Locale" />
      <div>
        <p className="eyebrow">Comune di Monza</p>
        <h1>Report Turno</h1>
        <p>Accesso riservato al personale di Polizia Locale.</p>
      </div>
    </div>    <form onSubmit={submit} className="loginForm">
...
const [mode, setMode] = useState(() => (auth?.ruolo === 'ufficiale' || auth?.ruolo === 'admin') ? 'ufficiale' : 'operatore');  const [report, setReport] = useState(loadReportDraft);
  ...
  async function loadReportsFromSupabase() {
    ...
    if (error) {
      console.error('Errore caricamento report Supabase:', error);
      return;
    }
  ...
  if (auth?.ruolo === 'ufficiale' || auth?.ruolo === 'admin') {
...
function loadTestReport() {
  const testReport = baseReport();

  testReport.data = '2026-08-14';
  testReport.turno = '06.30-13.30';
  testReport.orarioTipo = 'Ordinario';
  testReport.reparto = 'Radiomobile';
  testReport.zonaServizio = 'Pattuglia Radiomobile 1 – Centro, San Rocco, Triante';

  testReport.operatori = [
    {
      nome: 'FRANCESCA ONESTI',
      matricola: '9654',
      qualifica: 'Assistente Scelto'
    },
    {
      nome: 'MARIAGRAZIA PAPPALARDO',
      matricola: '11371',
      qualifica: 'Agente'
    },
    {
  ...
  testReport.veicoli = [
    {
      ...
      kmFine: '123584',
      carburante: 'Sì',
      importoCarburante: '52,30 €',
      oraPrelievoCard: '07.10',
      oraRestituzioneCard: '13.05',
      anomaliaVeicolo:
        'Fanalino posteriore destro non funzionante; verificare inoltre la pressione dello pneumatico anteriore sinistro.'
    },
    {
      sigla: 'Monza 12',
      kmInizio: '84520',
  ...
  testReport.interventi = [
    {
      ...emptyIntervento(),
      tipo: 'Sinistro stradale',
      origine: 'Centrale Operativa',
      oraInizio: '07.45',
      oraFine: '08.30',
      luogo: 'Via Borgazzi / Via Aquileia',
      descrizione:
        'Tamponamento tra due autovetture con un conducente che lamentava dolore cervicale.',
      conFeriti: 'Con feriti',
      veicoliCoinvolti: '2',
      rilievi: 'Sì'
    },
    ...
    {
      ...emptyIntervento(),
      tipo: 'Codice della strada',
      origine: 'UDT',
      oraInizio: '08.35',
      oraFine: '09.00',
      luogo: 'Via Lecco',
      descrizione:
        'Controllo soste in corrispondenza di attraversamento pedonale e fermata autobus.',
      cdsDettaglio: 'Controllo soste',
      cdsRimozione: 'Sì',
      cdsMotivazione:
        'Veicolo in sosta in posizione di grave intralcio.'
    },

    {
      ...emptyIntervento(),
      tipo: 'Posto di controllo',
      origine: 'Di iniziativa',
      oraInizio: '09.10',
      oraFine: '09.50',
      luogo: 'Viale Lombardia',
      descrizione:
        'Posto di controllo finalizzato alla verifica dei documenti di guida e circolazione.',
      veicoliControllati: '12',
      personeControllate: '15',
      verbaliElevati: '2',
      fermiSequestri: '0'
    },

    {
      ...
      oraInizio: '10.00',
      oraFine: '10.45',
      luogo: 'Via Cavallotti',
      descrizione:
        'Regolazione manuale della circolazione a seguito di temporaneo malfunzionamento semaforico.',
      motivoViabilita: 'Guasto semaforico',
      strade: 'Via Cavallotti, Via Volturno, Via Pavoni'
    },

    {
      ...emptyIntervento(),
      tipo: 'Servizio scuole',
      origine: 'ODS',
      scuole: [
        {
          nome: 'Scuola primaria Raiberti',
          momento: 'Uscita',
          orario: '12.30',
          criticita: ''
        },
        {
          nome: 'Scuola primaria Tacoli',
          momento: 'Uscita',
          orario: '12.40',
          criticita: ''
        },
        {
          nome: 'Scuola primaria Buonarroti',
          momento: 'Uscita',
          orario: '12.50',
          criticita: ''
        }
    ...
    {
      ...emptyIntervento(),
      tipo: 'Controllo autobus',
      origine: 'UDT',
      oraInizio: '13.05',
      oraFine: '13.40',
      luogo: 'Via Arosio',
      descrizione:
        'Controllo documentale e tecnico di autobus adibito al trasporto di studenti.',
      autobusControllati: '1',
      autobusVeicoloIdoneo: 'Si'
    },

    {
      ...emptyIntervento(),
      tipo: 'Verifica veicolo in stato di abbandono',
      origine: 'Centrale Operativa',
      oraInizio: '13.50',
      oraFine: '14.20',
      luogo: 'Via Monte Cimone',
      descrizione:
        'Verifica di veicolo segnalato da residente come apparentemente abbandonato sulla pubblica via.',
      abbandonoVerificato: 'Si',
      abbandonoEsito: 'Veicolo in stato di abbandono'
    },

    {
      ...emptyIntervento(),
      tipo: 'Rumori / disturbo quiete',
      origine: 'Centrale Operativa',
      oraInizio: '14.30',
      oraFine: '15.10',
      luogo: 'Via Bergamo',
      descrizione:
        'Intervento a seguito di segnalazione per musica ad alto volume proveniente da esercizio pubblico. Sul posto venivano effettuati accertamenti e contattato il responsabile dell’attività.'
    },

    {
      ...emptyIntervento(),
      tipo: 'Supporto ad altro ente',
      origine: 'UDT',
      oraInizio: '15.20',
      oraFine: '16.00',
      luogo: 'Via Marsala',
      descrizione:
        'Supporto operativo a personale di altro ente durante attività di verifica sul territorio, con presidio dell’area e assistenza nelle fasi di identificazione dei soggetti presenti.'
    },

    {
      ...
      tipo: 'Altro',
      origine: 'Altro',
      origineAltro: 'Segnalazione diretta di cittadino',
      oraInizio: '16.15',
      oraFine: '17.10',
      luogo: 'Piazza Trento e Trieste',
      descrizione:
        'Intervento richiesto a seguito di segnalazione diretta da parte di un cittadino che riferiva la presenza di più soggetti intenti a discutere animatamente nell’area pedonale della piazza. Giunti sul posto, gli operatori prendevano contatto con le persone presenti al fine di ricostruire quanto accaduto e procedevano alla loro identificazione. Nel corso degli accertamenti emergevano versioni parzialmente discordanti in merito all’origine della discussione, rendendo necessario acquisire ulteriori informazioni dai presenti e verificare l’eventuale presenza di testimoni. Veniva inoltre effettuato un controllo dell’area circostante per escludere situazioni di pericolo per i passanti e per verificare l’eventuale presenza di beni danneggiati o altri elementi utili. Al termine delle verifiche, dopo aver riportato la situazione alla calma e fornito ai soggetti le indicazioni del caso, venivano ripristinate regolari condizioni di sicurezza e fruibilità dell’area. Gli operatori rimanevano sul posto per il tempo necessario ad assicurarsi che non si verificassero ulteriori criticità e concludevano l’intervento dopo aver completato gli accertamenti di competenza.'
    }
  ];
  ...
  testReport.counters = {
    ...
    relazioni: 3,
    annotazioni: 2,
    sequestriAmministrativi: 1,
    fermiAmministrativi: 2,
    sequestriPenali: 1,
    cnr: 2,

    altriAttiNumero: 3,
    altriAttiDescrizione:
      'Elezione di domicilio, verbale di identificazione e dichiarazione di rinuncia al recupero del veicolo.',

    preavvisiCds: 8,
    vdcCds: 5,
    regPolizia: 2,
    regEdilizio: 1,
    regBenessereAnimali: 1,
    annonaria: 3,
    altreNorme: 2,
    altreNormeDescrizione:
      'Violazioni in materia ambientale e occupazione abusiva di suolo pubblico.',

    totaleVeicoliControllati: 34,
    totalePersoneControllate: 41
  };

  testReport.documentiRitirati = [
    {
      tipo: 'Patente',
      quantita: '1',
      note: 'Ritirata ai sensi del Codice della strada'
    },
    {
      tipo: 'Carta di circolazione',
      quantita: '2',
      note: 'Documenti acquisiti a seguito di accertamento'
    },
    {
      tipo: 'Autorizzazione / licenza',
      quantita: '1',
      note: 'Titolo acquisito per successive verifiche amministrative'
    }
  ];

  testReport.noteUdt =
    "Durante il turno l'attività si è svolta regolarmente, con particolare impegno nella gestione degli interventi connessi alla viabilità e al controllo del territorio. Si segnala la necessità di verificare il veicolo Monza 04 per il malfunzionamento del fanalino posteriore destro e per il controllo della pressione dello pneumatico anteriore sinistro. Alcuni interventi hanno richiesto tempi superiori alla normale attività ordinaria per la necessità di procedere ad accertamenti, identificazioni e verifiche documentali. Nulla di ulteriore da segnalare all'Ufficiale di coordinamento.";
...
const generatePeriodPdf = () => {
  ...
  <section className="card notice">
    ...
    <p>
      La Dashboard acquisisce automaticamente i report operatori salvati nel sistema.
      L'ufficiale verifica la sintesi generata, integra briefing, personale,
      attività ispettive, anomalie e note per il Comandante.
    </p>
...
<section className="card">
  ...
  {official.attivitaIspettive.map((a, idx) => (
    <div className="rowCard" key={idx}>
      <div className="grid four">
        <Field label="Tipologia verifica">
          <Input value={a.tipo} onChange={v => updateAttivita(idx, { tipo: v })} placeholder="es. controllo equipaggiamento, verifica presenza..." />
        </Field>
        <Field label="Pattuglia / reparto controllato">
          <Input value={a.reparto} onChange={v => updateAttivita(idx, { reparto: v })} />
        </Field>
        <Field label="Luogo controllo">
          <Input value={a.luogo} onChange={v => updateAttivita(idx, { luogo: v })} />
        </Field>
        <Field label="Orario">
          <Input value={a.orario} onChange={v => updateAttivita(idx, { orario: v })} />
        </Field>
      </div>

      <div className="grid three">
        <Field label="Esito">
          <Input value={a.esito} onChange={v => updateAttivita(idx, { esito: v })} />
        </Field>
        <Field label="Criticità rilevate">
          <Input value={a.violazioni} onChange={v => updateAttivita(idx, { violazioni: v })} />
        </Field>
        <Field label="Disposizioni impartite">
          <Input value={a.note} onChange={v => updateAttivita(idx, { note: v })} />
        </Field>
      </div>
  ...
  <button type="button" onClick={addAttivita}>
    + Aggiungi controllo UDT
  </button>
</section>
 <section className="card">
...
.compact-header .brand h1{
  font-size:13pt;
  margin:0;
}

.compact-header .brand p{
  font-size:7pt;
  margin-top:1mm;
  font-weight:700;
}
...
const eventiText = `${autoEventi || 'Nessun evento rilevante automatico rilevato.'}${official.eventiManuali ? '\n\nIntegrazione UDT sugli eventi:\n' + official.eventiManuali : ''}`;
  ...
  const page2 = `<section class="page">${headerHtml('REPORT UFFICIALE DI TURNO - DETTAGLIO', subtitle)}${panel('Violazioni riscontrate','table',violTable)}<div class="detail-grid-2">${panel('Atti redatti','clip',attiBody)}<div>${panel('Esito turno','check',`<p>${esc(official.esiti || '-')}</p>`)}${panel('Comunicazioni E.Q.','doc',`<p>${esc(official.comunicazioneEq || '-')}</p>`)}</div></div><div class="detail-grid-2">${panel('Nota per il comandante','clipboard',`<p>${esc(official.notaComandante || '-')}</p>`)}${panel('Responsabile di turno','user',`
  <div class="responsabile-box">
    <p class="center ruolo"><strong>${esc(official.qualifica || '-')}</strong></p>
    <p class="center nominativo"><strong>${esc(official.ufficiale || '-')}</strong></p>
    <div class="signature">Firma</div>
  </div>
`)}</div>${footerHtml(2)}</section>`;
...
const page2SintesiEndY = drawAutoTextPanel(
  doc,
  12,
  58,
  186,
...
function buildServicePdf(report) {
  ...
  drawPanel(doc,12,58,88,120,'Interventi effettuati','car',{accent:C.green}); let yy=77; doc.setFontSize(7.7); (report.interventi || []).slice(0, 9).forEach((i, idx) => {
  fillC(doc, c.green);
  ...
  if (i.tipo === 'Controllo autobus') {
    ...
    const idoneo = i.autobusVeicoloIdoneo || '-';
    testo += ` – Autobus controllati: ${n}. Veicolo idoneo: ${idoneo}.`;
  }

  if (i.tipo === 'Verifica veicolo in stato di abbandono') {
    const verifica = i.abbandonoVerificato || '-';
    const esito = i.abbandonoEsito || '-';
    testo += ` – Verifica effettuata: ${verifica}. Esito: ${esito}.`;
  }
